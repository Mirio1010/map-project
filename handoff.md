...
- **2026-05-10 (map flicker)** — Markers used **array index** React keys (`user-${index}` / `friend-${index}`); when `visibleMapPins` / `visibleMapFriendsPins` order or length changed, React remounted Leaflet markers → visible flicker. **Map** now keys markers by **`pin.id`** (UUID) when present. **`loadFriendsPins`** no longer clears friend state on a transient `getUser()` miss while `userIdForUi` is still set (avoids empty→full loops with auth). Friend fetch skips **`setFriendsPins`** when the ordered pin-id signature is unchanged; **`setFriendUsernames` / `setFriendColors` / `setFriendProfiles`** bail out when serialized data is unchanged so identical refetches do not re-render the map. There is **no** `setInterval` driving schedule checks — `visibleMapPins` / `visibleMapFriendsPins` remain **`useMemo`** on pin arrays only (time does not tick in deps; pins still appear/disappear on schedule only when parent state updates).
